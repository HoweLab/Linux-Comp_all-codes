...
import numpy as np
import os, time, pickle, sys, warnings
...
class L0_analysis:
    ...
    event_min_size : smallest allowable event in units of noise std [default: 3.0]
...
right
    ...
    def __init__(self, dataset,
                 event_min_size=3., noise_scale=.1, median_filter_1=5401, median_filter_2=101, halflife_ms=None,
...
#        if self._dff_traces is None and os.path.isfile(self.dff_file) and self.use_cache:
        if self._dff_traces is None and self.use_cache:
            ...
            for dff in dff_traces:
                ...
                num_small_baseline_frames.append(0)
            ...
            if self.use_cache: np.savez(self.dff_file, dff=dff_traces, noise_stds=np.array(noise_stds),
                                        num_small_baseline_frames=np.array(num_small_baseline_frames))
            self.print('done!')
 ...
 #          events = np.load(self.evfile)['ev']
        if True:
            ...
            for n, dff in enumerate(self.dff_traces[0]):
                ...
                if any(np.isnan(dff)):
                    ...
                    if self.use_bisection:
                        ...
                        print('using bracket')
                        (tmp, l) = self.bracket(tmp, self.dff_traces[1][n], 0, 10 * self.noise_scale, .0001,
                                                self.event_min_size)
                    print(f"lambda = {l}")
                    events.append(tmp)
            ...
            if self.use_cache:
                np.savez(self._evfile, ev=events)
    ...
    def bisection(self, dff, n, event_min_size, left=0, right=1, max_its=100, eps=.0001):
        ...
        tmp_right_l0 = self.l0(dff, self.gamma, right, self.L0_constrain, True)
        tmp_right_ev = tmp_right_l0['pos_spike_mag']
        tmp_right_t = tmp_right_l0['spikes']
        tmp_right = np.zeros(len(dff))
        tmp_right[tmp_right_t] = tmp_right_ev

        nz_right = (tmp_right > 0)

        # print('finding right endpoint')
        it = 0
        while it <= 20:
            it += 1
        ...
        print('right endpoint:{}'.format(right))
        # tmp_left = []
        # left = []

        print('finding lambda to minimize num events < min size')
        ...
        while it <= max_its:
            ...
            it += 1
            if (right - left) < eps:
                # print('right-left < eps')
                break

            mid = left + (right - left) / 2.

            tmp_left_l0 = self.l0(dff, self.gamma, left, self.L0_constrain, True)
            tmp_left_ev = tmp_left_l0['pos_spike_mag']
            tmp_left_t = tmp_left_l0['spikes']
            tmp_left = np.zeros(len(dff))
            tmp_left[tmp_left_t] = tmp_left_ev

            if (len(tmp_left) == 0) or (tmp_left is None):
                tmp_left = np.zeros((len(dff),))
            nz_left = (tmp_left > 0)
            ...
            if num_small_events_left == 0:
                # print('num_small_events_left = 0')
                break
            else:
                tmp_mid_l0 = self.l0(dff, self.gamma, mid, self.L0_constrain, True)
                tmp_mid_ev = tmp_mid_l0['pos_spike_mag']
                tmp_mid_t = tmp_mid_l0['spikes']
                tmp_mid = np.zeros(len(dff))
                tmp_mid[tmp_mid_t] = tmp_mid_ev
                ...
                if (len(tmp_mid) == 0) or (tmp_mid is None):
                    tmp_mid = np.zeros(len(dff))
